...
export default function About() {
  ...
  return (
    <section id="about" className="min-h-screen flex items-center px-8 md:px-20 py-32">
      <div ref={ref} className="max-w-7xl mx-auto w-full grid grid-cols-1 md:grid-cols-2 gap-20 items-center">
          ...
          <div className="pt-4 flex gap-12">
            {[["9", "Age I Started Coding"], ["2", "Real Systems Built"], ["1", "TV Appearance"]].map(([num, label]) => (
              <div key={label}>
                ...
                <p className="text-xs text-white/40 mt-1 max-w-[80px] leading-tight">{label}</p>
              </div>
            ))}
          </div>
        </motion.div>
